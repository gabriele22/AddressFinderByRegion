...
def read_conf_and_start(town):
    ...
    radius = data['radius_search_area']
    ...
    df_town.to_csv(f"address/Indirizzi_{town}.csv", sep=';', index=False, encoding='utf-8' )
...
def main():
    ...
    if len(towns_to_do) > 0:
        for t in towns_to_do:
            try:
                town_completed = read_conf_and_start(t)
